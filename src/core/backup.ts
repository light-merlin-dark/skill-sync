...
import type {
	BackupEntrySnapshot,
	BackupHarnessSnapshot,
	BackupManifest,
	BackupSkillFileSnapshot,
	HarnessDefinition,
	RuntimeContext,
	State,
} from "./types";
...
export function createBackup(
	...
	const harnessSnapshots = harnesses.map((harness) =>
		snapshotHarness(backupDir, harness),
	);
		...
		for (const name of readdirSync(harness.rootPath).sort()) {
			...
			if (stats.isSymbolicLink()) {
				...
				if (inspection.resolvedTarget) {
					snapshot.targetType = lstatSync(
						inspection.resolvedTarget,
					).isDirectory()
						? "directory"
						: "file";
					snapshot.skillFiles = collectSkillFiles(
						inspection.resolvedTarget,
						snapshot.targetType,
					);
				}
...
export function listBackups(runtime: RuntimeContext): BackupManifest[] {
	...
	return readdirSync(backupsDir)
		...
		.reverse()
		.map((id) =>
			readJsonFile<BackupManifest>(join(backupsDir, id, "manifest.json")),
		)
...
export function restoreBackup(
	...
	const manifest = readJsonFile<BackupManifest>(
	...
	const selected =
		selectedHarnessIds.length > 0
			? manifest.harnesses.filter((harness) =>
					selectedHarnessIds.includes(harness.id),
				)
			: manifest.harnesses;
	...
	for (const harness of selected) {
		...
		const currentEntries = existsSync(harness.rootPath)
			? readdirSync(harness.rootPath)
			: [];
	...
	if (backupState) {
		...
		for (const [entryPath, managed] of Object.entries(
			backupState.managedEntries,
		)) {
...
function restoreEntry(
	...
	throw new Error(
		`Cannot restore ${relative(harness.rootPath, destinationPath)}: no valid link target or backed-up SKILL.md files`,
	);
}

function restoreSkillFiles(
	...
	const treatAsDirectory =
		...
		entry.type === "symlink" ||
		entry.targetType === "directory" ||
		entry.skillFiles.some((file) => file.relativePath.includes("/"));
	if (treatAsDirectory) {
	...
	const [skillFile] = entry.skillFiles;
	if (!skillFile) {
		throw new Error(
			`Cannot restore ${destinationPath}: missing backed-up skill file`,
		);
	}
...
function collectSkillFiles(
	entryPath: string,
	entryType: "symlink" | "directory" | "file",
): BackupSkillFileSnapshot[] {
	...
	if (entryType === "file") {
		...
		return [
			{ relativePath: "SKILL.md", content: readFileSync(entryPath, "utf8") },
		];
	}
...
function walkForSkillFiles(
	...
	if (stats.isSymbolicLink()) {
		...
		if (absoluteCurrent.endsWith("SKILL.md")) {
			return [
				{
					relativePath: currentRelative || "SKILL.md",
					content: readFileSync(absoluteCurrent, "utf8"),
				},
			];
		}
		return [];
	...
	for (const name of readdirSync(absoluteCurrent).sort()) {
		...
		snapshots.push(
			...walkForSkillFiles(join(absoluteCurrent, name), nextRelative, visited),
		);
	}
	return snapshots;
}
